...
class Game:
        ...
        for x in range(8):
            for y in range(5,8):
                if x % 2 == 0 and y % 2 != 0:
                    ...
                    self.boardArr[x][y] = RED
        print("board in gameLogic:" , self.boardArr)
        print("id of board in gameLogic is ", id(self.boardArr))
        ...
        for piece in availableMoves:
            for destination in piece[1]:
                if counter == randomMoveIndex:
                    ...
                    reward = self.makeMove(piece[0], destination)
                    self.reverseBoard()
                    reward *= -1
                    return reward
    ...
    def makeMove(self, origin, destination):
        ...
        self.removeIfAttack(origin, destination)
    ...
    def getValidMoves(self, position):
        ...
        if self.getLeftDiagPos(position)[0] != -1 and self.getLeftDiagPos(position)[1] != -1:
        ...
        if self.getRightDiagPos(position)[0] != 8 and self.getRightDiagPos(position)[1] != -1:
